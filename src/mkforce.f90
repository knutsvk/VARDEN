...
contains
  ...
  subroutine mkvelforce(mla, vel_force, ext_vel_force, s, gp, lapu, visc, visc_fac, the_bc_tower)
    ...
    integer :: ng_f, ng_e, ng_g, ng_s, ng_l, ng_v
  ...
  subroutine mkvelforce_2d(vel_force, ext_vel_force, gp, s, lapu, visc, &
                           ng_f, ng_e, ng_g, ng_s, ng_l, ng_v, visc_fac, lo, hi)
    ...
    real(kind=dp_t), intent(in   ) ::          visc(lo(1)-ng_l:,lo(2)-ng_l:,:)
    real(kind=dp_t), intent(in   ) :: visc_fac
    ...
    integer :: i,j
       ...
       do j = lo(2), hi(2)
       do i = lo(1), hi(1)
          lapu_local(1:2) = visc(i,j,1) * visc_fac * lapu(i,j,1:2)
