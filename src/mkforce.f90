...
contains
  ...
  subroutine mkvelforce(mla, vel_force, ext_vel_force, s, gp, lapu, viscosity, visc_grad_term, &
    ...
    integer :: ng_f, ng_e, ng_s, ng_g, ng_l, ng_v
...
!                                sp(:,:,:,:), lp(:,:,:,:), vp(:,:,:,:), &
!                                ng_f, ng_e, ng_g, ng_s, ng_l, ng_v, visc_fac, lo, hi)
          end select
       end do
    enddo
  ...
  subroutine mkvelforce_2d(vel_force, ext_vel_force, gp, s, lapu, viscosity, visc_grad_term, &
                           ng_f, ng_e, ng_g, ng_s, ng_l, ng_v, visc_fac, lo, hi)

    use probin_module, only: boussinesq
 
    integer        , intent(in   ) :: lo(:), hi(:), ng_f, ng_e, ng_g, ng_s, ng_l, ng_v
    ...
    real(kind=dp_t), intent(in   ) :: visc_grad_term(lo(1)     :,lo(2)     :,:)
       ...
       do j = lo(2), hi(2)
          do i = lo(1), hi(1)
             ...
             vel_force(i,j,1:2) = vel_force(i,j,1:2) + &
                (lapu_local(1:2) + visc_grad_term(i,j,1:2) - gp(i,j,1:2)) / s(i,j,1)
          end do
       end do
  ...
  subroutine mkvelforce_3d(vel_force, ext_vel_force, gp, s, lapu, visc, &
    ...
    real(kind=dp_t), intent(in   ) ::          visc(lo(1)-ng_l:,lo(2)-ng_l:,lo(3)-ng_l:,:)
    real(kind=dp_t), intent(in   ) :: visc_fac
    ...
    integer :: i,j,k
       ...
       do j = lo(2), hi(2)
       do i = lo(1), hi(1)
          lapu_local(1:3) = visc(i,j,k,1) * visc_fac * lapu(i,j,k,1:3)
       ...
       do k=lo(3),hi(3)
          do j=lo(2),hi(2)
             lapu_local(1:3) = visc(lo(1),j,k,1) * visc_fac * lapu(lo(1),j,k,1:3)
       ...
       do k=lo(3),hi(3)
          do j=lo(2),hi(2)
             lapu_local(1:3) = visc(hi(1),j,k,1) * visc_fac * lapu(hi(1),j,k,1:3)
       ...
       do k=lo(3),hi(3)
          do i=lo(1),hi(1)
             lapu_local(1:3) = visc(i,lo(2),k,1) * visc_fac * lapu(i,lo(2),k,1:3)
       ...
       do k=lo(3),hi(3)
          do i=lo(1),hi(1)
             lapu_local(1:3) = visc(i,hi(2),k,1) * visc_fac * lapu(i,hi(2),k,1:3)
       ...
       do j=lo(2),hi(2)
          do i=lo(1),hi(1)
             lapu_local(1:3) = visc(i,j,lo(3),1) * visc_fac * lapu(i,j,lo(3),1:3)
       ...
       do j=lo(2),hi(2)
          do i=lo(1),hi(1)
             lapu_local(1:3) = visc(i,j,hi(3),1) * visc_fac * lapu(i,j,hi(3),1:3)
